...
def process_dir(cur_node: Node, next_dir: Dir, next_pos: Pos, matrix, visited):
    ...
    if is_valid_pos(next_pos, matrix) and (next_pos, next_movement) not in pending_visit:
        if is_same_movement_allowed(next_dir, cur_node.movement) or (cur_node.movement.dir != next_dir and cur_node.movement.same_dir_count >= MIN_SAME_DIR_COUNT):
            neighbor_distance = cur_node.dist + get_val(next_pos, matrix)
            changed = False
            if neighbor_distance <= dist[next_pos] and (next_dir == cur_node.movement.dir and cur_node.movement.same_dir_count >= MIN_SAME_DIR_COUNT):
...
MIN_SAME_DIR_COUNT = 4
MAX_SAME_DIR_COUNT = 10
...
while q:
    ...
    if dist[finish_pos] != float('inf'):
        print('early')
        break

    q.extend(neighbors) 

print('2:', dist[finish_pos])
