...
orig_rows_count = len(input)
orig_cols_count = len(input[0])
...
print('1:', solver([[1, 1], Dir.RIGHT]))

res = set()

# Top row
for i in range(1, orig_cols_count + 1):
    res.add(solver(([1, i], Dir.DOWN)))

# Bottom row
for i in range(1, orig_cols_count + 1):
    res.add(solver(([orig_rows_count, i], Dir.UP)))

# Left col
for i in range(1, orig_rows_count + 1):
    res.add(solver(([i, 1], Dir.RIGHT)))

# Right col
for i in range(1, orig_rows_count + 1):
    res.add(solver(([i, orig_cols_count], Dir.LEFT)))
